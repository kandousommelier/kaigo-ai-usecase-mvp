(() => {
  ...
  function planStateOf(row){
    const state=row?.answers?.planState;
    return state || {isDraft:false,reasons:[],unresolvedItems:[],quickWinMissing:[],quickWinChecked:[]};
  }
  ...
  function render(){const shown=rows.filter(r=>filter==="all"||r.status===filter);$("new-count").textContent=rows.filter(r=>r.status==="new").length;$("reviewed-count").textContent=rows.filter(r=>r.status==="reviewed").length;$("total-count").textContent=rows.length;$("submission-empty").hidden=shown.length>0;
    $("submission-list").innerHTML=shown.map(r=>`<article class="submission-card ${r.status==='new'?'is-new':''}"><div class="submission-top"><div><span class="status-badge">${r.status==='new'?'未確認':'確認済み'}</span><span class="plan-status ${planStateOf(r).isDraft?'is-draft':'is-ready'}">${draftLabel(r)}</span><strong>${esc(r.work_category)}</strong></div><time>${fmt(r.created_at)}</time></div><dl><div data-facility-code="true"><dt>施設コード</dt><dd>${esc(facilityCodeOf(r))}</dd></div><div><dt>サービス</dt><dd>${esc(r.service_type)}</dd></div><div><dt>回答者</dt><dd>${esc(r.role_type)}</dd></div><div><dt>受付番号</dt><dd>${esc(r.receipt_number)}</dd></div></dl><div class="admin-extra"><div><dt>未確定</dt><dd>${(planStateOf(r).reasons||[]).length?"あり":"なし"}</dd></div><div><dt>クイックウィン</dt><dd>${quickWinLabel(r)}</dd></div><div><dt>振り返り日</dt><dd>${esc(reviewDateOf(r))}</dd></div><div><dt>未確定数</dt><dd>${(planStateOf(r).reasons||[]).length}</dd></div></div><button type="button" class="primary-button small" data-open="${r.id}">内容を確認する</button></article>`).join("");
    document.querySelectorAll("[data-open]").forEach(b=>b.onclick=()=>openDetail(b.dataset.open));
  }
  function openDetail(id){selected=rows.find(r=>r.id===id);if(!selected)return;const state=planStateOf(selected);$("detail-content").innerHTML=`<p class="eyebrow">${selected.status==='new'?'NEW PLAN':'REVIEWED PLAN'}</p><h2 id="detail-title">${esc(selected.work_category)}</h2><div class="detail-meta"><span data-facility-code="true">施設コード：${esc(facilityCodeOf(selected))}</span><span>${draftLabel(selected)}</span><span>${esc(selected.receipt_number)}</span><span>${fmt(selected.created_at)}</span><span>${esc(selected.service_type)}／${esc(selected.role_type)}</span><span>振り返り日：${esc(reviewDateOf(selected))}</span></div><section class="detail-section"><h3>未確定事項</h3>${list(state.reasons)}</section><section class="detail-section"><h3>クイックウィン適合チェック</h3><p>結果：${quickWinLabel(selected)}</p><h4>未確認・不適合の可能性がある項目</h4>${list(state.quickWinMissing)}</section><section><div class="output-card-head"><h3>AIへの依頼文</h3><button class="copy-button" data-copy="prompt">コピー</button></div><pre class="output-text">${esc(selected.ai_prompt)}</pre></section><section><div class="output-card-head"><h3>AI活用計画書</h3><button class="copy-button" data-copy="plan">コピー</button></div><pre class="output-text">${esc(selected.action_plan)}</pre></section><div class="detail-actions"><button class="secondary-button" data-copy="all">全文コピー</button><button class="primary-button" id="toggle-status">${selected.status==='new'?'確認済みにする':'未確認へ戻す'}</button><button class="danger-button" id="delete-one">削除する</button></div>`;
